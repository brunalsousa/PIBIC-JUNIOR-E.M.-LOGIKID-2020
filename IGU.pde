...
static class Raiz {
  ...
  static int xRetiraEscala(int x) {
    return round((x - Raiz.xDesloc) / Raiz.escala);
  }
  
  static int yRetiraEscala(int y) {
    return round((y - Raiz.yDesloc) / Raiz.escala);
  }

  static void mouseApertou(int botao, int x, int y) {
    x = Raiz.xRetiraEscala(x);
    y = Raiz.yRetiraEscala(y);
  ...
  static void mouseMoveu(int x, int y) {
    ...
    Componente c = igu.componenteNoPonto(x, y);
    if (c != null) {
      c.mouseMoveu(x - c.xAbs, y - c.yAbs);
